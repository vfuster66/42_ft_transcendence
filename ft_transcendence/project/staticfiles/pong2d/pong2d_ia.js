...
var Game = {
    ...
    endMatch: function (winner) {
        ...
        let url = winner === 'player' ? '/pong2d_success/' : '/pong2d_failure/';
    ...
    predictBallYPosition: function(ball, targetX) {
        let ballCopy = { ...ball };
        let iterations = 0; // Pour éviter une boucle infinie

        while (ballCopy.x > targetX && iterations < 1000) {
            ballCopy.x -= ballCopy.speed;
            ballCopy.y += (ballCopy.moveY === DIRECTION.UP ? -1 : 1) * (ballCopy.speed / 1.5);

            if (ballCopy.y <= 0) {
                ballCopy.y = -ballCopy.y;
                ballCopy.moveY = DIRECTION.DOWN;
            } else if (ballCopy.y >= Pong.canvas.height - ballCopy.height) {
                ballCopy.y = 2 * (Pong.canvas.height - ballCopy.height) - ballCopy.y;
                ballCopy.moveY = DIRECTION.UP;
            }
        ...
        if (iterations >= 1000) {
            console.error('predictBallYPosition reached maximum iterations');
        }
    ...
    initAI: function () {
        setInterval(function () {
            ...
            predictedY += (Math.random() - 0.5) * 50; // Ajouter une marge d'erreur
        ...
        }, 1000); // IA rafraîchit sa vue toutes les secondes
    }
};
